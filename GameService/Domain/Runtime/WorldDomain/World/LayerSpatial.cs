...
namespace Domain.Runtime.WorldDomain.World
{
    public class LayerSpatial
    {
        ...
        #region Methods
        public void Add(string entityId, int chunkX, int chunkY, int x, int y)
        {
            var chunk = GetOrCreateChunk(chunkX, chunkY);
            chunk.Add(entityId, x, y);
        }

        public void Remove(string entityId, int chunkX, int chunkY, int x, int y)
        {
            if (!chunks.TryGetValue((chunkX, chunkY), out var chunk))
                return;

            chunk.Remove(entityId, x, y);
        }
